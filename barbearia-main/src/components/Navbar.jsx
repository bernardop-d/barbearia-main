...
import { HomeIcon, CalendarIcon, PlusIcon, LogoutIcon } from './Icons'
    ...
    <>
      ...
      <header className="sticky top-0 z-40 bg-ink-900/95 backdrop-blur-md border-b border-ink-800">
        <div className="max-w-md mx-auto px-5 h-14 flex items-center justify-between">
          <span className="font-display text-2xl text-gradient tracking-widest">DUNGABARBER</span>
          <button
            onClick={logout}
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 px-4 py-2 rounded-2xl transition-all duration-200 active:scale-90
                  ${tab.highlight
                    ? 'bg-blade-500 text-ink-900 shadow-lg shadow-blade-500/30 -mt-4 px-5 py-3'
                    : active
                      ? 'text-blade-400'
              ...
              >
                ...
                <span className={`text-xs font-medium ${tab.highlight ? 'text-ink-900' : ''}`}>
                  {tab.label}
                </span>
              </button>
            )
